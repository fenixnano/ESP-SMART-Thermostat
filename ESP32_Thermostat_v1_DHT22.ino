...
String Year                 = "2023";     // For the footer line
...
void ReadSensor() {
  ...
  else
  {
    ...
    // aht.getEvent(&humidity, &temp);// populate temperature and humidity objects with fresh data
...
//#########################################################################################
void StartSensor() {
  Wire.setClock(100000);                           // Slow down the SPI bus for some aht280 devices
  if (!simulating) {                               // If not sensor simulating, then start the real one
    // bool status = aht.begin(); 
    // start dht sensor 
    bool status = dht.begin();        
    if (!status) {
      Serial.println("Could not find DHT sensor? Check wiring");
    }
    else {
